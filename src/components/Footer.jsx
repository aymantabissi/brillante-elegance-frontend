  ...
  return (
    <footer className="bg-white border-t border-stone-100">
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        {/* Information */}
        <div>
          <h4 className="text-xs font-bold tracking-[0.2em] uppercase text-stone-900 mb-5">Information</h4>
          <ul className="flex flex-col gap-3">
            {['Contact us', 'About Us', 'Order & Returns', 'FAQs'].map((item) => (
              <li key={item}>
        ...
        {/* Quick Shop */}
        <div>
          <h4 className="text-xs font-bold tracking-[0.2em] uppercase text-stone-900 mb-5">Quick Shop</h4>
          <ul className="flex flex-col gap-3">
            {[
              { label: 'Necklace', to: '/shop?cat=colliers' },
              { label: 'Bracelet', to: '/shop?cat=bracelets' },
              { label: 'Sunglass', to: '/shop?cat=lunettes' },
              { label: 'Promo',    to: '/shop?promo=true' },
            ].map((item) => (
              <li key={item.label}>
        ...
        {/* Customer Services */}
        <div>
          <h4 className="text-xs font-bold tracking-[0.2em] uppercase text-stone-900 mb-5">Customer Services</h4>
          <ul className="flex flex-col gap-3">
            {['Orders FAQs', 'Shipping', 'Privacy Policy', 'Return & Refund'].map((item) => (
              <li key={item}>
                <Link to="/" className="text-sm text-stone-500 hover:text-stone-900 transition">{item}</Link>
              </li>
            ))}
      ...
      <div className="border-t border-stone-100 px-6 py-5">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <p className="text-xs text-stone-400">2026 Brillante Elegance. All Rights Reserved.</p>
